...
%% 第七步：执行码片速率时钟与载波对齐测试
% 执行IEEE 802.15.4a标准16.4.6节规定的码片速率时钟与载波对齐测试
% 测试内容包括：
%   1. 码片调制时钟精度测试 - 精度要求±20×10⁻⁶
%   2. 载波中心频率精度测试 - 精度要求±20×10⁻⁶
% 使用1MHz分辨率带宽和1kHz视频带宽进行测量
fprintf('\n开始执行码片速率时钟与载波对齐测试...\n');

uwb_ChipRateCarrierAlignment(waveHPRF, cfgHPRF,true);  % 默认不显示图形输出

fprintf('码片速率时钟与载波对齐测试完成\n');

%% 第八步：执行发射中心频率容差测试
% 执行IEEE 802.15.4a标准16.4.9节规定的发射中心频率容差测试
% 测试内容包括：
%   1. 发射中心频率容差测试 - 容差要求±20×10⁻⁶
%   2. 频率稳定性检查
% 该要求应从16.4.6节中码片时钟容差的要求
fprintf('\n开始执行发射中心频率容差测试...\n');

uwb_TransmitCenterFrequencyTolerance(waveHPRF, cfgHPRF);  % 默认不显示图形输出

fprintf('发射中心频率容差测试完成\n');

%% 测试完成
fprintf('\n===============================\n');
fprintf('所有UWB HPRF测试项目已完成\n');
fprintf('===============================\n');
